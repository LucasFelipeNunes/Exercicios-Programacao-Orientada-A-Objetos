...
public class Lista0Exercicio05{
	public static void main (String[] args) {
		...
		double N = 0, M = 0;
		do{
			System.out.print("Digite a dimensao inteira e positiva N de elementos nas linhas das matrizes a serem digitadas:\t");
			N = ler.nextDouble(); 
			if((N*10)%10 != 0 || N <= 0){
		...
		int[][] matriz1 = new int[(int)N][(int)M], matriz2 = new int[(int)N][(int)M], matriz3 = new int[(int)N][(int)M];
		for (int i = 0; i < N; i++){
			for (int j = 0; j < M; j++){
